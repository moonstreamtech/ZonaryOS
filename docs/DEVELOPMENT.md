...
- **Generic proxy routes**: `POST /api/workflow/instances` (create) and `POST /api/workflow/instances/{instanceId}/transitions/{actionKey}` (any transition) replace the old stock-specific `/api/stock/create` and `/api/stock/[instanceId]/sell` routes - same cookie-to-Bearer-token forwarding pattern, no authorization decision made in the route itself.
...
**Settings page** (`app/[locale]/settings/page.tsx`): firm name, the caller's role, and the firm's active workflow definitions - deliberately read-only. The `firms` table has a `name` and a jsonb `attributes` column, but no HTTP handler anywhere mutates either today; this page shows only what a real request can back, rather than promising editing the backend doesn't support yet.
