...
// Vision §3's financial management core: chart-of-accounts management -
// create, toggle active, view hierarchy. Owner-gated the same way
// settings/permissions/page.tsx is (isOwner, not a permission key - see
// that page's own doc comment for why "authorized" means is_owner in this
// codebase for firm-structural settings pages).
export default async function AccountsSettingsPage({ params }: PageProps) {
  const { locale } = await params;
  setRequestLocale(locale);
  const t = await getTranslations("AccountsSettings");
  ...
  const accounts = await fetchAccounts(sessionToken, firm.firmId);
  ...
  return (
    <main className="flex flex-1 flex-col items-center gap-10 bg-zinc-50 px-6 py-16 dark:bg-black">
      ...
      {accounts === null ? (
        ...
        <AccountsManager firmId={firm.firmId} accounts={accounts} />
      )}
    </main>
  );
}
