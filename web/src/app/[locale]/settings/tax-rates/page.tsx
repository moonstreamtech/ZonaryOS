...
import { fetchTaxRates } from "@/lib/localization";
...
// localization) - firm-scoped tax rate list, manual entry only (item 24
// remains open for real VAT/GST calculation logic and fiscal reporting;
// this is the foundation table plus the invoice-line override wired in
// internal/invoicing.resolveLineTaxRate). Owner-gated the same way
// settings/addresses/page.tsx is.
export default async function TaxRatesSettingsPage({ params }: PageProps) {
  const { locale } = await params;
  setRequestLocale(locale);
  const t = await getTranslations("TaxRatesSettings");
  ...
  const taxRates = await fetchTaxRates(sessionToken, firm.firmId);
  ...
  return (
    <main className="flex flex-1 flex-col items-center gap-10 bg-zinc-50 px-6 py-16 dark:bg-black">
      ...
      {taxRates === null ? (
        ...
        <TaxRatesManager firmId={firm.firmId} taxRates={taxRates} />
      )}
    </main>
  );
}
