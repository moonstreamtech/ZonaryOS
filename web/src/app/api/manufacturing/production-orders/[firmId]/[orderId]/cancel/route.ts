...
// Proxies the Go backend's POST .../production-orders/{orderId}/cancel
// (owner-only) - no authorization decision made here:
// internal/manufacturing.CancelProductionOrder is the sole place that
...
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  const result = await cancelProductionOrder(token, firmId, orderId);
  if (!result.ok) {
