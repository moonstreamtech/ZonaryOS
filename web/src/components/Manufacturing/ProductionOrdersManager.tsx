...
import type { ProductionOrder, BOM } from "@/lib/manufacturing";
...
// The /manufacturing/orders page's order list - mirrors
// components/SalesOrders/SalesOrdersManager.tsx's own create-form/row
// shape. The BOM dropdown is filtered to the chosen product's own BOMs
// (client-side, from the full boms list already fetched) - matching
// internal/manufacturing.CreateProductionOrder's own server-side rejection
// of a BOM that doesn't belong to the given product.
export default function ProductionOrdersManager({ firmId, orders, boms, products, isOwner }: Props) {
  ...
  const bomsForProduct = productId ? boms.filter((b) => b.productId === productId) : [];

  async function submitCreate(e: FormEvent) {
    e.preventDefault();
    setCreateError(null);
    if (!productId || !bomId || !quantityPlanned) {
    ...
    setSubmitting(true);
    try {
      const res = await fetch(`/api/manufacturing/production-orders/${firmId}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ productId, bomId, quantityPlanned }),
      });
      if (!res.ok) {
              ...
              >
                ...
                {bomsForProduct.map((b) => (
                  <option key={b.id} value={b.id}>
                    {b.name} ({b.version}){b.isActive ? ` - ${t("active")}` : ""}
                  </option>
                ))}
              </select>
            </label>
            ...
            <table className="w-full border-collapse text-left text-sm">
              <thead>
                <tr className="border-b border-zinc-300 text-zinc-600 dark:border-zinc-700 dark:text-zinc-400">
                  ...
                  <th className="py-2 pr-4 font-medium">{t("quantityPlanned")}</th>
                </tr>
              </thead>
              <tbody>
                {orders.map((o) => (
                  ...
                  >
                    ...
                    <td className="py-2 pr-4">{productName(products, o.productId)}</td>
                    <td className="py-2 pr-4">{t(`status.${o.status}`)}</td>
                    <td className="py-2 pr-4 font-mono">{o.quantityPlanned}</td>
                  </tr>
                ))}
              </tbody>
            </table>
