...
// The /sales-orders page's order list - mirrors
// components/Invoicing/InvoicesManager.tsx's own create-form/row shape:
// create is owner-gated (server-checked by internal/salesorders' own
// CreateSalesOrder; isOwner here only controls what renders), the create
// form is deliberately minimal - one line per order, matching
// InvoicesManager's own "functional, not polished" first pass. An order
// auto-created by stock_to_sale's record_sale transition (the
// workflow-to-sales-order bridge) appears in this same list with no
// visual distinction beyond its orderNumber - clicking through to its own
// detail page (which shows sourceWorkflowInstance) is where that
// provenance becomes visible.
export default function SalesOrdersManager({ firmId, orders, customers, isOwner }: Props) {
  ...
  const visibleOrders = filter.trim()
    ? orders.filter((o) => o.orderNumber.toLowerCase().includes(filter.trim().toLowerCase()))
    : orders;
  ...
  async function submitCreate(e: FormEvent) {
    ...
    if (!trimmedDescription || !quantity || !unitPrice) {
    ...
    setSubmitting(true);
    try {
      const res = await fetch(`/api/salesorders/${firmId}`, {
        method: "POST",
